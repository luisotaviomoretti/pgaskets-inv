-- 017_fix_audit_call.sql
-- Qualify audit function with schema and set search_path to ensure resolution
...
BEGIN
  ...
  BEGIN
    IF v_movement.type = 'RECEIVE' THEN
      ...
      RETURNING jsonb_build_object('layer_id', id, 'remaining_quantity', remaining_quantity, 'original_quantity', original_quantity)
      INTO v_layer_info;
    ...
    SELECT public.log_movement_deletion(
      v_movement,
      'REVERSE',
      p_deletion_reason,
      p_deleted_by,
    ...
    v_result := jsonb_build_object('success', true, 'movement_id', p_movement_id, 'movement_type', v_movement.type, 'restored_layers', v_restored_layers, 'reversed_at', NOW(), 'audit_id', v_audit_id);
    RETURN v_result;
