--- Migration: Deprecate Hard Delete Functions and Replace with Soft Delete
-- This migration safely transitions from hard delete to soft delete
-- Created: 2025-08-20

-- 1) Rename existing hard delete functions (for safety/rollback)
-- Check and rename delete_movement if it exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'delete_movement' AND pg_get_function_arguments(oid) = 'p_movement_id integer, p_deletion_reason text DEFAULT NULL::text, p_deleted_by text DEFAULT ''system''::text') THEN
    ALTER FUNCTION delete_movement(integer, text, text) RENAME TO delete_movement_hard_deprecated;
  END IF;
END
$$;

-- Check and rename reverse_movement if it exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'reverse_movement' AND pg_get_function_arguments(oid) = 'p_movement_id integer, p_deletion_reason text DEFAULT NULL::text, p_deleted_by text DEFAULT ''system''::text') THEN
    ALTER FUNCTION reverse_movement(integer, text, text) RENAME TO reverse_movement_hard_deprecated;
  END IF;
END
$$;

-- Check and rename delete_production_group if it exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'delete_production_group' AND pg_get_function_arguments(oid) = 'p_reference text, p_deletion_reason text DEFAULT NULL::text, p_deleted_by text DEFAULT ''system''::text') THEN
    ALTER FUNCTION delete_production_group(text, text, text) RENAME TO delete_production_group_hard_deprecated;
  END IF;
END
$$;

-- 2) Create new reverse_movement function that uses soft delete
...
-- 3) Create new delete_movement function that uses soft delete
...
-- 4) Create new delete_production_group function that uses soft delete
...
-- 5) Create admin functions for emergency hard delete (with warnings)
...
-- 6) Create function to bulk soft delete movements by criteria
...
-- 9) Create migration status function
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE proname LIKE '%_hard_deprecated'
  ) INTO v_functions_migrated;
  ...
  RETURN jsonb_build_object(
    ...
    'soft_delete_percentage', 
      CASE WHEN v_total_movements > 0 
        ...
        ELSE 0 
      END,
    'old_functions_preserved', v_functions_migrated,
    'can_rollback', v_functions_migrated
  );
END;
$$;
